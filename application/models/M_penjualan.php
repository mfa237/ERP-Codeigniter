...
class M_penjualan extends CI_Model{
  ...
  function get_all_item_($cabang, $Gudangdisplay){
   ...
   function get_all_item($cabang, $Gudangdisplay, $kategori_id){
     $wherekategori = "";
     if ($kategori_id != null) {
       $wherekategori =  "WHERE a.m_category_2_id = '$kategori_id'";
     }

    $query = $this->db->query("SELECT a.*, b.det_promo_status_aktif, c.*,
                               IFNULL(g.stok_gudang_jumlah, 0) AS stockgudang FROM m_barang a
                               LEFT JOIN m_detail_promo b ON b.barang_id = a.barang_id LEFT JOIN m_promo c ON c.promo_id = b.promo_id
                               LEFT JOIN ( SELECT a.stok_gudang_jumlah, a.m_barang_id FROM t_stok_gudang a
                                           LEFT JOIN m_gudang b ON b.gudang_id = a.m_gudang_id
                                           WHERE b.m_cabang_id = '$cabang' AND b.gudang_id != '$Gudangdisplay' ) AS g ON
                                           g.m_barang_id = a.barang_id $wherekategori");
    return $query;
  }
